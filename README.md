...
## AdaptiveBitrateServiceTests

The `AdaptiveBitrateServiceTests` class provides unit tests for adaptive bitrate streaming functionality, including streaming profile ladder generation, resolution validation, bitrate calculations, and pipeline settings validation. It verifies that adaptive streaming configurations work correctly for various source resolutions and encoding scenarios.

Here is an example usage of the `AdaptiveBitrateServiceTests` class with its public members:
...
// Test that ladder generation for a 1080p source contains expected renditions
[Fact]
...
// Test that low-res source does not upscale to higher resolutions
[Fact]
...
// Test that <see cref="StreamingProfile.TotalBitrateKbps"/> correctly calculates combined video and audio bitrate
[Fact]
...
// Test that <see cref="StreamingProfile.Resolution"/> correctly formats width and height as resolution string
[Fact]
...
// Test that <see cref="StreamingSegment.ActualBitrateKbps"/> returns zero when duration is zero to avoid division by zero
[Fact]
...
// Test that <see cref="StreamingSegment.ActualBitrateKbps"/> correctly calculates bitrate from file size and duration
[Fact]
